...
{       
	...
	if(is_search() && get_option('WidgetMaster_On_Search') == 'NO') {return $params;}
        ...
        if(isset($_SESSION[$params['0']['widget_id']]) && $_SESSION[$params['0']['widget_id']] == 'closed')
        {	
        ...
        if(!empty(get_option('WidgetMaster_Border_Color'))) {$WidgetMaster_Border_Color = "1px solid #".get_option('WidgetMaster_Border_Color');}
        else{$WidgetMaster_Border_Color = 'none';}
        
        if(!empty(get_option('WidgetMaster_Icon_Bg_color'))) {$WidgetMaster_Icon_Bg_color = "#".get_option('WidgetMaster_Icon_Bg_color');}
        else{$WidgetMaster_Icon_Bg_color = 'none';}
        
        if(!empty(get_option('WidgetMaster_Icon_color'))) {$WidgetMaster_Icon_color = "#".get_option('WidgetMaster_Icon_color');}
    ...
    if(isset($_GET['widgetid'])){
        ...
        if(!isset($_SESSION[$_GET['widgetid']])){
                $_SESSION[$_GET['widgetid']] = 'closed';
                
                if( !get_option( "WM_widget_name_".urldecode($_GET['widgetname']) ) ) {
                    add_option("WM_widget_name_".urldecode($_GET['widgetname']), "1");
                } else {
                    $plusOne = get_option("WM_widget_name_".urldecode($_GET['widgetname'])) + 1;
                    update_option("WM_widget_name_".urldecode($_GET['widgetname']), $plusOne );
                }
                
        }
        else{
                unset($_SESSION[$_GET['widgetid']]);
        }
